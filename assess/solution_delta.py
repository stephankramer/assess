...
to the Stokes equations in cylindrical and spherical shell domains with a delta function RHS forcing term,
which in the 2D cylindrical shell domain takes the form:

.. math::

  f = -g \delta(r-r') cos(n\phi) \hat{r}

where r is the radius (distance from origin), \phi is the angle with the x-axis, r' is the radius
of the density anomaly, and \hat{r} is the outward pointing radial unit vector. The magnitude g,
degree l and wave number n can be chosen arbitrarily. Similarly in a 3D spherical domain we
consider the forcing term

.. math::

  f = -g \delta(r-r') Y_lm(\theta,\phi) \hat{r}
...
and return four coefficients A, B, C, D for the linear combination of biharmonic
spherical functions that constitute the analytical solution.

This module has been automatically generated by extracting the solutions from the
latex source of the associated paper. If sign=+1 we use the following substitutions

  alpha_pm = Rp/rp, alpha_mp = Rm/rp, pm=+1, mp=-1

i.e. we take the top case for the symbols \alpha_\pm, \alpha_\mp, \pm, and \mp in the paper
to obtain :math:`A_+, B_+, C_+,` and :math:`D_+` corresponding to the upper half of the domain. If sign=-1
we have

  alpha_pm = Rm/rp, alpha_mp = Rp/rp, pm=-1, mp=+1

to obtain the :math:`A_-, B_-, C_-,` and :math:`D_-` coefficients of the lower half of the solution.
"""

__all__ = []


def solution_cylinder_delta_fs(Rp, Rm, rp, n, g, nu, sign):
...
def solution_cylinder_delta_ns(Rp, Rm, rp, n, g, nu, sign):
...
def solution_sphere_delta_fs(Rp, Rm, rp, l, g, nu, sign):
    alpha_pm, alpha_mp = [Rp/rp, Rm/rp][::int(sign)]
    pm = sign
...
def solution_sphere_delta_ns(Rp, Rm, rp, l, g, nu, sign):
    alpha_p, alpha_m = [Rp/rp, Rm/rp]
    alpha_pm, alpha_mp = [Rp/rp, Rm/rp][::int(sign)]
    pm, mp = sign, -sign
